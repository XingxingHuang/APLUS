...
class colorCatalog:
    ...
    def run(self,band): 
        ...
        for i in range(ncats):
            ...
            basefits= imname[0:j] + '.fits'     #    by xingxing  for the catalog name: a209_f110w_drz_nir.cat
		...
		filter=fUtil.filterName(fitsfile)   # by xingxing
                ...
                #    filter = fUtil.filterNameWFC3(fitsfile)   # all above by xingxing
                if filter:
            ...
            if (len(t1) != nsources):
                self.logfile.write("Catalog dimension mismatch: "+str(len(t1))+' '+str(nsources))
        ...
        for i in range(len(header)):
            ...
            if len(line) < 3:
                ...
                if (line[3]+"_"+line[4]) not in xFilts:
                    xFilts.append(line[3]+"_"+line[4])     # too long. it will be delete  by XX
                continue
            elif line[3] in ignoreFilters:
                if line[3] not in badFilts:
                    badFilts.append(line[3])   # in ignoreFilters. it will be writen to badFilts
                continue
            elif line[-2] != "MAG":
                continue
            elif line[-1] != "BPZ":
                continue
            else:                     # like this one   "HST_WFC3_IR_F105W_MAG_BPZ"
