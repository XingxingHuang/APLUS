...
#   chi2=chi2-minchi2
   ...
   p=where(greater_equal(chi2,1400.),0.,exp(-chi2/2.))
...
def prior(z,m,info='hdfn',nt=6,interp=0,x=None,y=None):
    ...
    forma='%.'+accuracy+'f'
    m_dict=forma %m    
    if not prior_dict.has_key(m_dict) or info=='lensing': #if lensing, the magnitude alone is not enough
        if info<>'lensing':
	    ...
	    nz=pp_i.shape[0]
	    nt=pp_i.shape[1]
	    nti=nt+(nt-1)*int(interp)
	    tipos=arange(nt)*1.
	    itipos=arange(nti)*1./(1.+float(interp))
