...
__version__      = '$Revision: 1.9 $ '[11:-3]
__version_date__ = '$Date: 2010/05/22'
__author__       = 'J. Blakeslee, R. White, K. Anderson, L. Bradley, W. Zheng'
...
class gscMatchup:
    ...
    def applyCorrs(self):
        ...
        for imlist in [self.sciImlist, self.ctxImlist, self.wgtImlist, self.rmsImlist]:
            for im in imlist:
                ...
                ff[0].header.update('REFBAND','None')
                filt=ff[0].header.get('FILTER1') #WZ
                if string.find(filt,'CLEAR') > -1:
                    filt=ff[0].header.get('FILTER2')
                if (string.find(self.ref.band,filt) > -1 or self.skip==None): #WZ
                    ...
                    ff[0].header.update('REFBAND',filt)            
    ...
    def makeObjcats(self):
        ...
        for imsci in self.sciImlist:
            ...
            # cmd = 'project_coords %s 2 3 %.6f %.6f outfile=%s asec' \
            #       %(objcat,self.RActr,self.Decctr,projcat)
            self.logfile.write("**>  "+cmd)
