...
pc=3.0856e18      #pc in cm
mpc=3.0856e24     #mpc in cm
...
def reobs(sed,m=0.,z_0=0.,oldfilter='I_LRIS',z_new=0.,newfilter='V_LRIS',cosmology=(0.3,0.7,.7)):
	...
	zo,f_mod_0=get_data(model_path,(0,1))
        ...
        for i in range(len(z_new)):
            fnew[i]=match_resol(zo,f_mod_0,z_new[i])            
    else:
	fnew=f_z_sed(sed,newfilter,z_new,units='nu')
        
    fnew=where(equal(fnew,0.),99.,fnew) #if the new flux is 0, returns 99. (code non-detection) 
    ...
    k=2.5*log10(((1.+z_new)/fnew)*(f_old/(1.+z_0)))
    ...
    #Distance modulus
#    print cosmology
#    print dist_mod(z_new,cosmology)
#    print dist_mod(z_0,cosmology)
