...
class photoZ:
	...
	self.bpzCat     = self.obsCats+'/bpz_'+Band+'.cat' #WZ
	self.errorList  = []
	self.inputList  = []
	self.outputList = {}

        cname='multicolor_'+Band+'.cat'
	self.inputList.append(cname) # WZ 'multicolor.cat')
        cname='multicolor_'+Band+'.columns'
	self.inputList.append(cname) # 'multicolor.columns')

    def runBpz(self,Band):
	...
	file = self.obsCats+'/multicolor_'+Band+'.columns' 
	...
	cmd = 'python ' + bpz + self.colorCat + ' -P $BPZPATH/bpzCLASH.param -VERBOSE no -INTERP 2 -DZ 0.005 -SPECTRA eB11.list' #WZ
        print cmd
        # pdb.set_trace()
	try:
