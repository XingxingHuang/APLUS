...
# $Id: catalog.py,v 2.0.0 2011/08/09 21:22:36 Zheng Exp $
# ---------------------------------------------------------------------

__version__      = '$Revision: 2.00 $ '[11:-3]
__version_date__ = '$Date: 2011/08/08 21:22:36 $ '[7:-3]
...
class Catalog: 
	...
	self.detectionImage = os.path.join(self.obsFits,'detection_'+obs.band+'.fits')   #by xingxing
	...
	#pdb.set_trace()       
	if (string.find(string.upper(obs.detector),'WFC') > -1  and string.find(string.upper(obs.detector),'IR')==-1) :
            if richField:
            ...
            if richField:
                self.defaultInParFile = os.path.join(obs.pardir,self.modName+'_uvis_richfield.inpar')
                self.logfile.write("Selected rich field UVIS input parameter set.")
            else:
                self.defaultInParFile = os.path.join(obs.pardir,self.modName+'_uvis_sparsefield.inpar') 
                self.logfile.write("Selected UVIS sparse field input parameter set for SExtractor.")
        elif (string.find(string.upper(obs.detector),'IR') > -1):
            if richField:
                self.defaultInParFile = os.path.join(obs.pardir,self.modName+'_ir_richfield.inpar')
                self.logfile.write("Selected rich field IR input parameter set.")
            else:
                self.defaultInParFile = os.path.join(obs.pardir,self.modName+'_ir_sparsefield.inpar') 
                self.logfile.write("Selected sparse field IR input parameter set for SExtractor.")
        else:
            self.defaultInParFile = os.path.join(obs.pardir,self.modName+'.inpar')
            self.logfile.write("Detector is not WFC,HRC or UVIS,IR. Using default input parameter set.")
    ...
    def setpars(self,fitsfile,band):
        ...
        catalogFileName = os.path.join(self.obsCats, fitsname +'_'+band+ '.cat')   #by xingxing  band added
        inParFileName   = os.path.join(self.obsPars, fitsname +'_'+band+ '.inpar')
        paramFileName   = os.path.join(self.obsPars, fitsname +'_'+band+ '.param')
        ...
        im=pyfits.open(fitsfile,'update') #WZ
        im[0].header.update('ZEROPT',zpoint)
        im.close()
