...
class filterCatalog(Catalog):
    ...
    def _magFix(self,catalogFile):
        ...
        # WZ: Preseve decimal points for RA/Dec (the last two columns)
        ...
        for i in range(len(selectSet)):
            if len(selectSet[i]) == 2:
                ...
                elif "J2000" in paramNames: #WZ
                    #wcsCols.append((column,name))
                    wcsField = tableio.get_str(catalogFile,(column-1))
                    newWCSList.append(wcsField)
                    newWCSHeaders.append(name)
                else:
        ...
        newHeaders   = newHeaders   + newWCSHeaders #WZ

        newVariables = tuple(newFieldList)
        newVariables2 = tuple(newWCSList)
        ...
        fob.write('## This proprietary file was generated by the CLASH Pipeline.\n##\n')
        ...
        fob.close()
        tableio.put_data2(catalogFile,newVariables,newVariables2,append="yes")

        return

    def detFix(self,reffile):
        """Read detectionImage.cat and compare every item with that in the reference catalog.
           output rows that have a match
           Nov 2010, WZ
        """
        # n,x,y = tableio.get_data(reffile,(1,2,3))
        n = tableio.get_str(reffile,0)
        curdir = os.getcwd()
        #catdir=self.obsdir+'/Catalogs'
        os.chdir(self.obsCats)
        #cmd = 'cp detectionImage.cat detectionImage.cat.old'
        #sproc  = popen2.Popen3(cmd,1)
        os.rename('detectionImage.cat',"detectionImage.cat.old")
        n0=n1=0
        f0 = open("detectionImage.cat.old",'r')
        f1 = open("detectionImage.cat",'w')
        for line in f0:
            head = string.split(line)[0]
            if (string.find(head,'#') > -1):
                f1.write(line)
            else:
                i=int(head)
                flag=-1.0
        ...
        f0.close()
        f1.close()
        print "DetectionImage.cat: Write "+str(n1)+' sources out of '+str(n0)
        os.chdir(curdir)
        return
