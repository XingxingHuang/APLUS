...
def ask(what):
    """Usage:
       ans=ask(pregunta)
    This function prints the string pregunta, 
...
def gauss_int(x=arange(0.,2.5,.5),average=0.,sigma=1.):
    #Brute force approach and limited accuracyfor >3sigma
    #find something better
    #DO NOT USE IN LOOPS (very slow)
    """Calculates the (0,x) integral of a gaussian"""
    uno=0
    if type(x)==type(0.):
	x=array([x])
	uno=1
    if sometrue(less(x,0.)): 
	print 'No negative x values allowed'
	sys.exit()
    step=.00001
    xn=arange(0.,x[-1]+step,step)
    gn=1./sqrt(2.*pi)*exp(-xn**2/2.)
    z=(x-average)/sigma
    ind=searchsorted(xn,z)
